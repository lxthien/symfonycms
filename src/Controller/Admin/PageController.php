...
class PageController extends AbstractController
{
     ...
     */
    public function editAction(Request $request, $id, Slugger $slugger, RedirectManager $redirectManager, MediaSelectionManager $mediaSelection, NewsMediaManager $newsMediaManager)
    {
        $this->denyAccessUnlessGranted('CMS_CONTENT_EDIT');
        $news = $this->findPage($id);
    ...
    {
        $page = $this->getDoctrine()->getRepository(News::class)->find($id);

        if (!$page || !$page->isPage()) {
            throw $this->createNotFoundException('Page không tồn tại.');
        }
