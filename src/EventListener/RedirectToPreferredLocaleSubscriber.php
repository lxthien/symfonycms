...
class RedirectToPreferredLocaleSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event)
    {
        ...
        if (0 === mb_stripos($referer, $request->getSchemeAndHttpHost())) {
            return;
        }
        ...
        if ($preferredLanguage !== $this->defaultLocale) {
            $targetUrl = $this->urlGenerator->generate('homepage', ['_locale' => $preferredLanguage]);
            $targetPath = parse_url($targetUrl, PHP_URL_PATH) ?: '/';

            if ($targetPath === $request->getPathInfo()) {
                return;
            }
